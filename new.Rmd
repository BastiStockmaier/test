...
This shows that we if we take a random vector, transformation with the covariance matrix of the dataset will move this vector in both direction and lenght. Interestingly if we would now use the correlation matrix to transform the resulting vector again we would approach a vector where the slope of the vector i.e its direction doesn't change anymore when transformed by the covariance matrix. This specific vector is called an eigenvector:
...
We will only use two dimensions in this example so we can plot the data at each step to visualize what the PCA does. Imagine we went to a bat cave and we measured eyesize and nostrile size of different individuals. 
...
Again this is pretty simple using the cov() function in R. Since the data is two dimensional, the covariance matrix will be 2 x 2. 
```{r}
